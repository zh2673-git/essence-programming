...
| 文件路径 | 类型定义 | 核心字段 | 约束规则 |
|---------|---------|---------|---------|
| {path} | {Class/Interface} | {fields} | {constraints} |

### 2. 数据存储
| 数据类型 | 存储介质 | 存储策略 | 持久化机制 |
|---------|---------|---------|-----------|
| {type} | {DB/Cache/File} | {sync/async} | {backup strategy} |
...
| 接口名 | 输入类型 | 输出类型 | 调用方式 |
|-------|---------|---------|---------|
| {name} | {input} | {output} | {sync/async/event} |
```
...
对每个需深入分析的模块，必须按四层结构输出设计文档：

```markdown
### {模块名} 四层设计

#### 1. 数据规矩
- **状态定义**：State/TypedDict/Dataclass 代码
- **字段说明**：每个字段的类型、约束、默认值
- **验证规则**：字段级别的校验逻辑

#### 2. 数据存储
- **存储介质**：数据库表名/缓存键模式/文件路径
- **持久化策略**：同步写入 vs 异步写入
- **备份/恢复**：数据持久化保障机制

#### 3. 数据流转
- **状态转换图**：状态机可视化（Mermaid）
- **流转方法**：每个状态转换对应的方法
- **验证点**：流转前的校验规则
- **异常处理**：流转失败的回滚机制
